...
import { Plus, Trash2, Package, ShieldCheck, ArrowLeft } from "lucide-react";
...
function AdminPage() {
  ...
  const [admins, setAdmins] = useState<Product[]>(() => listAdminProducts());
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const newId = "A" + Math.floor(Math.random() * 9_000_000 + 1_000_000);
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap mb-8">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Manage your Glamora catalogue — add products and they appear instantly across the store.
          </p>
        </div>
        <div className="flex gap-3">
          <Stat label="Total products" value={totalCount} />
          <Stat label="Your additions" value={admins.length} />
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        >
          ...
          <button className="w-full bg-primary text-primary-foreground rounded-full py-3 font-medium hover:opacity-90">
            Add Product
          </button>
        </motion.form>

        {/* Existing admin products list */}
        <div className="lg:col-span-2 bg-card border rounded-2xl p-6">
          <h2 className="font-semibold text-lg flex items-center gap-2 mb-4">
            <Package className="size-5 text-primary" /> Your Products ({admins.length})
          </h2>
          {admins.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-12">
              No products added yet.<br />Use the form to add your first one.
            </p>
          ) : (
            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
              {admins.map(p => (
                <div key={p.id} className="flex gap-3 border rounded-xl p-3">
                  ...
                  <button onClick={() => remove(p.id)} className="text-destructive hover:bg-destructive/10 rounded-lg size-8 grid place-items-center shrink-0">
