...
function ProfilePage() {
  const { user, setUser, addresses, saveAddress, removeAddress, orders, wishlist } = useStore();
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10 space-y-6">
          ...
          <div className="space-y-3">
            {addresses.map((a, i) => (
              <div key={i} className="flex justify-between gap-3 border rounded-xl p-3 text-sm">
                ...
                <button onClick={() => removeAddress(i)} className="text-muted-foreground hover:text-destructive"><Trash2 className="size-4" /></button>
              </div>
            ))}
          </div>
        )}
      ...
      <section className="grid sm:grid-cols-3 gap-4">
        ...
        <button onClick={() => { setUser(null); toast.success("Logged out"); }} className="bg-card border rounded-2xl p-5 hover:border-destructive text-left"><p className="font-medium text-destructive">Logout</p><p className="text-sm text-muted-foreground">Sign out of your account</p></button>
      </section>
    </div>
  );
}
